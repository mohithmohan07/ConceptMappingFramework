...
function BlockCard({ title, selected, hint, onClick }: { title: string; selected?: boolean; hint?: string; onClick?: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function ColumnPanel({ title, count, empty, children }: { title: string; count?: number; empty?: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
      <div className="mb-2 flex items-center justify-between">
        <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">{title}</h3>
        {typeof count === "number" && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">{count}</span>}
      </div>
...
export function FlowChartPage({ tree, onBack }: Props) {
  ...
  // Focus gaps selection chain: Grade -> Chapter -> Topic -> Parent Concept -> Concept
  ...
  const [fgParent, setFgParent] = useState<string | null>(null);
  ...
  const fgChapters = useMemo(() => Array.from(new Set(filtered.filter((c) => c.grade === fgGrade).map((c) => c.chapter))).sort((a, b) => a.localeCompare(b)), [filtered, fgGrade]);
  const fgTopics = useMemo(() => Array.from(new Set(filtered.filter((c) => c.grade === fgGrade && c.chapter === fgChapter).map((c) => c.topic))).sort((a, b) => a.localeCompare(b)), [filtered, fgGrade, fgChapter]);
  const fgParents = useMemo(() => Array.from(new Set(filtered.filter((c) => c.grade === fgGrade && c.chapter === fgChapter && c.topic === fgTopic).map((c) => c.parentConcept))).sort((a, b) => a.localeCompare(b)), [filtered, fgGrade, fgChapter, fgTopic]);
  const fgConcepts = useMemo(() => filtered.filter((c) => c.grade === fgGrade && c.chapter === fgChapter && c.topic === fgTopic && c.parentConcept === fgParent), [filtered, fgGrade, fgChapter, fgTopic, fgParent]);
  ...
  const relatedConcepts = useMemo(() => {
    ...
    return filtered
      .filter((c) => c.id !== selectedConcept.id && c.parentConcept === selectedConcept.parentConcept && c.chapter === selectedConcept.chapter)
      .slice(0, 8);
  }, [selectedConcept, filtered]);

  const resetSelections = () => {
    setFgGrade(null); setFgChapter(null); setFgTopic(null); setFgParent(null);
      ...
      <div className="mx-auto max-w-[1760px] space-y-4">
        <header className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="text-sm text-slate-500">Restructured with logic-first hierarchy and cleaner drill-down.</p>
            </div>
            <div className="flex gap-2">
              <button type="button" onClick={resetSelections} className="rounded-full border border-slate-300 bg-white px-4 py-2 text-sm">Reset selection</button>
          ...
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-4">
            <TabButton active={tab === "focus-gaps"} onClick={() => setTab("focus-gaps")} label="Focus Gaps Flow Chart" subtitle="Grade → Chapter → Topic → Parent → Concept" />
            <TabButton active={tab === "library"} onClick={() => setTab("library")} label="Library Flow Chart" subtitle="Parent Concept → Concepts with grade/chapter" />
            <TabButton active={tab === "curriculum"} onClick={() => setTab("curriculum")} label="Curriculum Flow Chart" subtitle="Grade → Chapter → Topics" />
            <TabButton active={tab === "concept-overview"} onClick={() => setTab("concept-overview")} label="Concepts - Parent Concepts Flow Chart" subtitle="Grouped concepts + downward lineage" />
          </div>
        </header>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          ...
          <div className="grid gap-2 lg:grid-cols-[1.4fr_repeat(4,minmax(140px,1fr))]">
            ...
            <SelectField label="Grade" value={filters.grade} options={options.grades} onChange={(value) => setFilters({ ...filters, grade: value })} />
            <SelectField label="Subject" value={filters.subject} options={options.subjects} onChange={(value) => setFilters({ ...filters, subject: value })} />
            <SelectField label="Chapter" value={filters.chapter} options={options.chapters} onChange={(value) => setFilters({ ...filters, chapter: value })} />
            <SelectField label="Parent Concept" value={filters.parentConcept} options={options.parentConcepts} onChange={(value) => setFilters({ ...filters, parentConcept: value })} />
          </div>
        </section>

        {tab === "focus-gaps" && (
          <section className="grid gap-3 xl:grid-cols-5">
            <ColumnPanel title="Grade" count={grades.length}>{grades.map((g) => <BlockCard key={g} title={`Grade ${g}`} selected={fgGrade===g} onClick={() => { setFgGrade(g); setFgChapter(null); setFgTopic(null); setFgParent(null); }} />)}</ColumnPanel>
            <ColumnPanel title="Chapter" count={fgChapters.length} empty={fgChapters.length===0 ? "Select grade first." : undefined}>{fgChapters.map((ch) => <BlockCard key={ch} title={ch} selected={fgChapter===ch} onClick={() => { setFgChapter(ch); setFgTopic(null); setFgParent(null); }} />)}</ColumnPanel>
            <ColumnPanel title="Topic" count={fgTopics.length} empty={fgTopics.length===0 ? "Select chapter first." : undefined}>{fgTopics.map((t) => <BlockCard key={t} title={t} selected={fgTopic===t} onClick={() => { setFgTopic(t); setFgParent(null); }} />)}</ColumnPanel>
            <ColumnPanel title="Parent Concept" count={fgParents.length} empty={fgParents.length===0 ? "Select topic first." : undefined}>{fgParents.map((p) => <BlockCard key={p} title={p} selected={fgParent===p} onClick={() => setFgParent(p)} />)}</ColumnPanel>
            <ColumnPanel title="Concepts" count={fgConcepts.length} empty={fgConcepts.length===0 ? "Select parent concept first." : undefined}>{fgConcepts.map((c) => <BlockCard key={c.id} title={c.name} hint={`Gap ${c.gapScore ?? "-"} · Coverage ${c.coverage ?? "-"}`} selected={selectedConceptId===c.id} onClick={() => setSelectedConceptId(c.id)} />)}</ColumnPanel>
          </section>
        )}

        {tab === "library" && (
          <section className="grid gap-3 xl:grid-cols-[330px_1fr]">
            <ColumnPanel title="Parent Concepts" count={libraryParents.length}>{libraryParents.map((p) => <BlockCard key={p} title={p} selected={libraryParent===p} onClick={() => setLibraryParent(p)} />)}</ColumnPanel>
            <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
              <div className="mb-2 flex items-center justify-between"><h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">Concepts</h3><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">{libraryConcepts.length}</span></div>
              <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{libraryConcepts.map((c) => <BlockCard key={c.id} title={c.name} hint={`Grade ${c.grade} · Chapter ${c.chapter}`} selected={selectedConceptId===c.id} onClick={() => setSelectedConceptId(c.id)} />)}</div>
            </div>
          </section>
        )}
        ...
        {tab === "curriculum" && (
          <section className="grid gap-3 xl:grid-cols-3">
            <ColumnPanel title="Grade" count={grades.length}>{grades.map((g) => <BlockCard key={g} title={`Grade ${g}`} selected={curGrade===g} onClick={() => { setCurGrade(g); setCurChapter(null); }} />)}</ColumnPanel>
            <ColumnPanel title="Chapter" count={curChapters.length} empty={curChapters.length===0 ? "Select grade first." : undefined}>{curChapters.map((c) => <BlockCard key={c} title={c} selected={curChapter===c} onClick={() => setCurChapter(c)} />)}</ColumnPanel>
            <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"><div className="mb-2 flex items-center justify-between"><h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">Topics</h3><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">{curTopics.length}</span></div><div className="grid gap-2 sm:grid-cols-2">{curTopics.map((t) => <BlockCard key={t} title={t} />)}</div>{curChapter && curTopics.length===0 && <p className="mt-2 text-xs text-slate-500">No topics in this selection.</p>}</div>
          </section>
        )}
        ...
        {tab === "concept-overview" && (
          <section className="grid gap-3 xl:grid-cols-[1.5fr_1fr]">
            <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm space-y-4">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-slate-500">All Concepts (grouped by Grade → Chapter)</h3>
              {groupedConcepts.map(([grade, chapterMap]) => (
                <div key={grade} className="rounded-xl border border-slate-200 p-3">
                  ...
                  {Array.from(chapterMap.entries()).sort((a,b)=>a[0].localeCompare(b[0])).map(([chapter, items]) => (
                    <div key={`${grade}:${chapter}`} className="mb-3">
                      <p className="mb-1 text-xs font-semibold uppercase text-slate-500">Chapter: {chapter}</p>
                      <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{items.map((c) => <BlockCard key={c.id} title={c.name} hint={c.parentConcept} selected={selectedConceptId===c.id} onClick={() => setSelectedConceptId(c.id)} />)}</div>
                    </div>
                  ))}
                </div>
              ))}
            ...
            <div className="space-y-3">
              <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                ...
                {!selectedConcept && <p className="text-sm text-slate-500">Select a concept to view Concept → Topic → Chapter → Parent Concept flow.</p>}
                {selectedConcept && (
                  <div className="space-y-2">
              ...
              {selectedConcept && (
                <div className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
                  ...
                  <div className="space-y-2">
                    ...
                    {relatedConcepts.map((c) => <BlockCard key={c.id} title={c.name} hint={`Same parent/chapter · ${c.topic}`} onClick={() => setSelectedConceptId(c.id)} />)}
                  </div>
                </div>
              )}
            </div>
