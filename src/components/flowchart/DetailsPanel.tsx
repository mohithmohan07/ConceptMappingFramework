...
function ValueRow({ label, value }: { label: string; value: string | undefined }) {
  return (
    <div className="flex items-start justify-between gap-2 rounded-lg bg-slate-50 px-3 py-2 text-sm">
...
export function DetailsPanel({ selectedNode, conceptIndex, onJumpTab }: Props) {
  ...
  return (
    <aside className="w-full rounded-2xl border border-slate-200 bg-white p-4 shadow-sm xl:w-[340px]">
      ...
      {selectedNode && (
        <div className="mt-3 space-y-3">
          ...
          <div className="space-y-2">
            <ValueRow label="Chapter" value={selectedConcept?.chapter ?? (selectedNode.meta?.chapter as string | undefined)} />
            <ValueRow label="Topic" value={selectedConcept?.topic ?? (selectedNode.meta?.topic as string | undefined)} />
            <ValueRow
              label="Parent Concept"
              value={selectedConcept?.parentConcept ?? (selectedNode.meta?.parentConcept as string | undefined)}
            />
          ...
          <div>
            ...
            <div className="grid gap-2">
              <button
                type="button"
                onClick={() => onJumpTab("focus-gaps")}
                className="rounded-lg border border-slate-200 px-3 py-1.5 text-left text-xs text-slate-700 hover:bg-slate-50"
              >
                Open in Focus Gaps Flow Chart
              </button>
              <button
                type="button"
                onClick={() => onJumpTab("library")}
                className="rounded-lg border border-slate-200 px-3 py-1.5 text-left text-xs text-slate-700 hover:bg-slate-50"
              >
                Open in Library Flow Chart
              </button>
              <button
                type="button"
                onClick={() => onJumpTab("curriculum")}
                className="rounded-lg border border-slate-200 px-3 py-1.5 text-left text-xs text-slate-700 hover:bg-slate-50"
              >
                Open in Curriculum Flow Chart
              </button>
            </div>
          </div>
        </div>
